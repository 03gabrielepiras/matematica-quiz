...
function addP0(){
  BANK.push({
    ...
    choices:['3/4','5/6','Sono uguali'],
    a:1,
    rule: ruleBox('Confronto frazioni (incrocio)','Confronta a*d e b*c.','3*6=18 e 4*5=20 ⇒ 5/6 è maggiore.')
  });
  BANK.push({
...
function checkOpen(){
  ...
  if(ok){
    ...
    let why = '';
    if(mode==='train'){
      if(root && root.rule) why = `<br><span class="muted">Suggerimento:</span> ${renderMathInline(root.rule.split('\n')[0])}`;
    ...
    $('feedback').innerHTML = `❌ <strong>No</strong>. <span class="muted">Corretto:</span> <strong>${renderMathInline(q.a)}</strong>${why}`;
    wrongLog.push({q: root?root: q, user:$('answer').value});
  }
