...
function buildQueue(blockId, modeWanted){
  ...
  function questionHasSkill(q, sid){
    if(!q) return false;
    if(q.type === "guided"){
      if(Array.isArray(q.skills) && q.skills.indexOf(sid) !== -1) return true;
      if(Array.isArray(q.steps)){
        for(var i=0;i<q.steps.length;i++){
          if(q.steps[i] && q.steps[i].skill === sid) return true;
        }
      }
      return false;
    }
  ...
  for(var i=0;i<b.skills.length;i++){
    var sid = b.skills[i];
    if(!skillGreen(profile, sid)) notGreen.push(sid);
  }

  var out = [];

  // 1) 1 domanda per ogni skill non verde (se possibile)
  var shuffledNotGreen = shuffle(notGreen);
  for(var j=0;j<shuffledNotGreen.length;j++){
    var s = shuffledNotGreen[j];
    var candidates = bank.filter(function(q){ return questionHasSkill(q, s); });
  ...
  var pool = bank.filter(function(q){ return questionHasSkill(q, targetSkill); });
...
function startVerify(){
  // Verifica sul blocco che l’utente ha usato/selezionato per ultimo
  const bid = profile.lastBlock || pickTrainingBlock(profile);
