...
function makeEmptyProfile(){
  return {
    ...
    streak: 0,
    badges: {},
    // completamento "a livello blocco" (passando la verifica)
    passed: {},
    lastBlock: "M1"
  };
}
...
function blockUnlocked(profile, id){
  ...
  if(!b) return false;
  // prereq blocks must be completed.
  // Regola semplice: basta "passare" la verifica del prerequisito.
  // (In alternativa, vale anche il completamento per skills verdi.)
  for(const pre of b.prereq){
    const pb = blockById(pre);
    if(!pb) return false;
    if(!(profile.passed && profile.passed[pre]) && !allGreen(profile, pb.skills)) return false;
  }
  return true;
}
...
function blockCompleted(profile, id){
  const b = blockById(id);
  if(!b) return false;
  return (profile.passed && profile.passed[id]) || allGreen(profile, b.skills);
}
...
function guidedProductExample(){
  ...
  return qGUIDED({
    ...
    steps:[
      ...
      qMCQ({
        block:"M2", skill:"M2_C9",
        prompt:"Se invece (−2x^2)(+3x^3), che segno ha il risultato?",
        choices:["Positivo","Negativo"],
        answerIndex:1,
        rule:"Nel prodotto: segni diversi → risultato negativo; segni uguali → positivo.",
        example:"(−2)·(+3)=−6"
      }),
      qMCQ({
        block:"M2", skill:"M2_C6",
...
function guidedDivisionExample(){
  ...
  return qGUIDED({
    ...
    steps:[
      ...
      qMCQ({
        block:"M2", skill:"M2_C9",
        prompt:"Se invece (−6x^5) / (+2x^3), che segno ha il risultato?",
        choices:["Positivo","Negativo"],
        answerIndex:1,
        rule:"Nella divisione: segni diversi → risultato negativo; segni uguali → positivo.",
        example:"(−6)/(+2)=−3"
      }),
      qMCQ({
        block:"M2", skill:"M2_C7",
...
function guidedPowerExample(){
  ...
  return qGUIDED({
    ...
    steps:[
      ...
      qMCQ({
        block:"M2", skill:"M2_C9",
        prompt:"Se invece (−2x^3)^2, che segno ha il risultato?",
        choices:["Positivo","Negativo"],
        answerIndex:0,
        rule:"Potenza pari di un numero negativo → risultato positivo.",
        example:"(−2)^2=+4"
      }),
      qMCQ({
        block:"M2", skill:"M2_C8",
...
function startVerify(){
  // Verifica sul blocco usato/selezionato per ultimo.
  // Se non c'e', usa la logica di suggerimento.
  var bid = profile.lastBlock || pickTrainingBlock(profile);

  // P0 sempre verificabile
  if(bid === "P0"){
    startBlock("P0", "verify");
    return;
  }
  ...
  // Mantieni vincolo P0 prima di M2/M5
...
function finishBlock(){
  // Se è una verifica e la passi, sblocca il blocco successivo.
  // Regola super semplice: >=80% in verifica = "passato".
  if(mode === "verify" && queue && queue.length){
    const ratio = score / queue.length;
    if(ratio >= 0.8){
      if(!profile.passed) profile.passed = {};
      profile.passed[currentBlock] = true;
      saveProfile(profile);
      toast(`✅ Verifica superata su ${currentBlock}! (${Math.round(ratio*100)}%)`);
    } else {
      toast(`❌ Verifica non superata (${Math.round(ratio*100)}%). Riprovaci!`);
    }
  } else {
