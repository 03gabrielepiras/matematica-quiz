...
async function preloadAllBanks(){
  ...
  const blocks = ['P0','M1','M2','M3','M4','M5'];
  for(const b of blocks){
    try{ await loadBankFromJSON(b); }catch{}
  }
}
...
function renderDashboard(){
  ...
  drow.innerHTML = `
    <div class="dashRowTop">
      <div><strong>🎯 Oggi</strong> <span class="muted">obiettivo</span></div>
      <div class="tag">${done}/${goal}</div>
    </div>
    <div class="bar"><div style="width:${pctToWidth(dpct)}"></div></div>
    <div class="dashMeta">Consiglio: fai ${Math.max(0, goal-done)} domande per completare l'obiettivo.</div>
  `;
...
/* -------------------- Keypad -------------------- */
function applyKeypadPrefs(){
  const input = $('openInput');
  const pad = $('keypad');
  const s = (profile && profile.settings) ? profile.settings : makeEmptyProfile().settings;
  const usePad = (s.useKeypad !== false);
  const sysKbd = !!s.systemKeyboard;
  if(pad) pad.classList.toggle('hidden', !usePad);
  if(input){
    // If we want only the custom keypad, try to prevent system keyboard on mobile.
    input.setAttribute('inputmode', sysKbd ? 'decimal' : 'none');
    input.readOnly = !sysKbd; // avoids keyboard pop when tapping input (iOS)
  }
}
...
function renderKeypad(){
  ...
  function smartInsert(token){
    ...
    // auto-multiplication: 2x, x(, )(, 3(, x2 -> x*2
    const first = raw.charAt(0);
    const prevIsAtom = /[0-9a-zA-Z)]/.test(prev);
    const nextIsAtom = /[0-9a-zA-Z(]/.test(first);
    const insertingOp = /[+\-*/^)]/.test(first);

    let toInsert = raw;
    if(prev && prevIsAtom && nextIsAtom && !insertingOp){
      // avoid duplicates around operators
    ...
    setCursor(start + toInsert.length);
  }
  ...
  layout.forEach(row => {
    row.forEach(k => {
      ...
      b.addEventListener('click', (e)=>{
        e.preventDefault();
        if(k.t === 'DEL') return del();
        if(k.t === 'CLR') return clearAll();
        if(k.t === '=') return; // no-op placeholder
        smartInsert(k.t);
      });
      pad.appendChild(b);
    });
...
function submitOpen(){
  ...
  if(ok){
    ...
    if(meta.equivalence && q.answer && profile.settings && profile.settings.suggestStandard){
      ...
      $("feedback").textContent = "✅ Corretto (forma equivalente). Forma standard: " + std + warn;
    } else {
...
function submitGuidedOpen(){
  ...
  if(ok && meta.equivalence && step.answer && profile.settings && profile.settings.suggestStandard){
    ...
    const warn = (mode === 'verify' && profile.settings.warnStandardInVerify) ? ' ⚠️ In verifica si consiglia la forma standard.' : '';
    $("feedback").textContent = "✅ Ok (forma equivalente). Forma standard: " + std + warn;
  } else {
    ...
    switch(action){
      ...
      case 'open-settings':
        setNavActiveSafe('navSettings');
        {
          ...
          const opt6=$('optUseKeypad'); if(opt6) opt6.checked = !!(profile.settings && profile.settings.useKeypad);
          const opt7=$('optSystemKeyboard'); if(opt7) opt7.checked = !!(profile.settings && profile.settings.systemKeyboard);
      ...
      case 'save-keypad-settings':
        if(!profile.settings) profile.settings = {};
        {
          const k1 = $('optUseKeypad');
          const k2 = $('optSystemKeyboard');
          profile.settings.useKeypad = k1 ? !!k1.checked : true;
          profile.settings.systemKeyboard = k2 ? !!k2.checked : false;
          // apply immediately
          applyKeypadPrefs();
          saveProfile(profile);
          toast('Impostazioni tastierino salvate.');
        }
        break;
      case 'start-errors':
        closeModal('settingsModal');
