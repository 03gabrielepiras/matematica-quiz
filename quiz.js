...
function importContentFromObject(obj){
  ...
  if(!v.ok){ setContentStatus('❌ '+v.msg); toast('JSON contenuti non valido.'); return; }

  const res = applyContentPayload(obj);
  if(!res.ok){ setContentStatus('❌ '+res.msg); toast(res.msg); return; }
...
function importContentFromText(){
  ...
  try{
    ...
    setContentStatus('❌ JSON non valido.');
    toast('JSON non valido.');
  }
}
...
function importContentFromFile(file){
  ...
  reader.onload = () => {
    try{
      ...
      setContentStatus('❌ File JSON non valido.');
      toast('File JSON non valido.');
    }
  };
  reader.onerror = () => { setContentStatus('❌ Impossibile leggere il file.'); toast('Impossibile leggere il file.'); };
  reader.readAsText(file);
}
...
_bind('validateContentBtn',()=>{
  ...
  try{
    const obj=JSON.parse(raw);
    const v=validateContentObject(obj);
    setContentStatus((v.ok?'✅ ':'❌ ')+v.msg);
  }catch{ setContentStatus('❌ JSON non valido.'); }
});
